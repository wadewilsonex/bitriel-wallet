...
async function signMessage(candidate){

  const value = "5000000000000000000";
  const expiredAt = Date.now() + (15 * 86400000);
  const privateKey = "0x6c0cb7bec296427954b73aad36c28e1dccfebd4025dd880519d743b682d77102";

  const web3 = new Web3('https://data-seed-prebsc-1-s1.binance.org:8545');

  return new Promise(async (resolve, reject) => {
    // const 
    const h = web3Utils.soliditySha3(
      value,
      candidate,
      expiredAt
    )
    ...
    const r = web3Utils.hexToBytes(sig.r);
    const s = web3Utils.hexToBytes(sig.s);

    resolve({sig, value, expiredAt, r, s});

    // const candidateInfo = await WhiteL
  });
}
