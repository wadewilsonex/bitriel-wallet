...
function getLatestMeta(registry, json) {
  const metadata = registry.createType('ContractMetadata', (0, _util.isObject)(json.V2) ? {
    V2: json.V2
  } : (0, _util.isObject)(json.V1) ? {
    V1: json.V1
  } : {
    V0: json
  });
  return metadata.isV2 ? metadata.asV2 : metadata.isV1 ? (0, _toLatest.v1ToLatest)(registry, metadata.asV1) : (0, _toLatest.v0ToLatest)(registry, metadata.asV0);
}
